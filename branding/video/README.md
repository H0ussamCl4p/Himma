...
| 3.2–7.5s | **Kinetic tagline** | "Forge **ahead.**" slams in word-by-word (blur + spring), push-in, → ember panel wipe |
...
| 19.4–22s | **End card** | Mascot + wordmark + "Join the waitlist" pill + forge-get.com, breathing glow |
...
npm run render       # → out/forge-brand.mp4
```
...
Edit `src/ForgeBrand.tsx` (scenes, timings, copy) and `src/brand.ts` (colors).
Frame math: 30fps — 1 second = 30 frames. Scene boundaries are set in the
`<Sequence from={...}>` blocks at the bottom of `ForgeBrand.tsx`.
